 ...
 */
#ifndef BLE_MAC
#define BLE_MAC FALSE // 是否自定义蓝牙Mac地址 ( 默认:FALSE - 使用芯片Mac地址 )，需要在main.c修改Mac地址定义
#endif
#ifndef DCDC_ENABLE
...
#define SLEEP_RTC_MIN_TIME US_TO_RTC(1000) // 非空闲模式下睡眠的最小时间（单位：一个RTC周期）
#endif
#ifndef SLEEP_RTC_MAX_TIME
#define SLEEP_RTC_MAX_TIME MS_TO_RTC(RTC_TO_MS(RTC_TIMER_MAX_VALUE) - 1000 * 60 * 60) // 非空闲模式下睡眠的最大时间（单位：一个RTC周期）
#endif
#ifndef WAKE_UP_RTC_MAX_TIME
#define WAKE_UP_RTC_MAX_TIME US_TO_RTC(1400) // - 等待32M晶振稳定时间（单位：一个RTC周期），根据不同睡眠类型取值可分为： 睡眠模式/下电模式: 45(默认); 暂停模式: 45; 空闲模式: 5
#endif
#ifndef TEM_SAMPLE
#define TEM_SAMPLE TRUE // 是否打开根据温度变化校准的功能，单次校准耗时小于10ms( 默认:TRUE )
#endif
#ifndef BLE_CALIBRATION_ENABLE
#define BLE_CALIBRATION_ENABLE TRUE // 是否打开定时校准的功能，单次校准耗时小于10ms( 默认:TRUE )
#endif
...
#endif
#ifndef CLK_OSC32K
#define CLK_OSC32K 1 // RTC时钟选择，如包含主机角色必须使用外部32K( 0 外部(32768Hz)，默认: 1：内部(32000Hz)，2：内部(32768Hz) ). 该项请勿在此修改，必须在工程配置里的预处理中修改，如包含主机角色必须使用外部32K
#endif
...
#endif
#ifndef BLE_TX_POWER
#define BLE_TX_POWER LL_TX_POWEER_0_DBM // 发射功率( 默认:LL_TX_POWEER_0_DBM (0dBm) )
#endif
#ifndef PERIPHERAL_MAX_CONNECTION
#define PERIPHERAL_MAX_CONNECTION 1 // 最多可同时做多少从机角色( 默认:1 )
#endif
#ifndef CENTRAL_MAX_CONNECTION
#define CENTRAL_MAX_CONNECTION 3 // 最多可同时做多少主机角色( 默认:3 )
#endif
