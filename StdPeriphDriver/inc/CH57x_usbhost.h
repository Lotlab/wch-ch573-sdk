...
uint8_t   EnableRootHubPort(void);                          // 使能ROOT-HUB端口,相应的bUH_PORT_EN置1开启端口,设备断开可能导致返回失败
...
uint8_t   USBHostTransact( uint8_t endp_pid, uint8_t tog, uint32_t timeout );  // endp_pid: 高4位是token_pid令牌, 低4位是端点地址
uint8_t   HostCtrlTransfer( uint8_t* DataBuf, uint8_t* RetLen );  // 执行控制传输,8字节请求码在pSetupReq中,DataBuf为可选的收发缓冲区
// 如果需要接收和发送数据,那么DataBuf需指向有效缓冲区用于存放后续数据,实际成功收发的总长度返回保存在ReqLen指向的字节变量中


void CopySetupReqPkg( const char* pReqPkt );            // 复制控制传输的请求包
uint8_t CtrlGetDeviceDescr( void );                    // 获取设备描述符,返回在 pHOST_TX_RAM_Addr 中
uint8_t CtrlGetConfigDescr( void );                    // 获取配置描述符,返回在 pHOST_TX_RAM_Addr 中
